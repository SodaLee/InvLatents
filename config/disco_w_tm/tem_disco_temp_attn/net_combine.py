...
class TemConvNet(Net):
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        ref_cond = inputs["full_pose_img_seq"] # (B, 4C, T, H, W)
        ...
        # b, c, t, h, w = latents.size()
        # sh = (b, c, 1, h, w)
        # base_noise = torch.randn(sh).to(device=self.device, dtype=self.dtype).repeat(1, 1, self.args.nframes, 1, 1)
        # noise = base_noise_ratio * base_noise + (1 - base_noise_ratio) * res_noise
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        #     generator,
        #     latents=None,
        # ).repeat(1, 1, self.args.nframes, 1, 1)
        # latents = base_noise_ratio * base_latents + (1 - base_noise_ratio) * res_latents
        latents = res_latents
