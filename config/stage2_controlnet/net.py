...
class Net(nn.Module):
    def __init__(
        ...
        self.latents_porj = torch.nn.Sequential(
            #torch.nn.Linear(clip_image_encoder.visual_projection.out_features, (224//self.vae_scale_factor)**2, bias=False),
    ...
    def gen_ref_noise(self, image, generator, h = 0, w = 0):
        if h != 0 and w != 0:
            image = torch.nn.functional.interpolate(image, size=(h, w), mode="bilinear", align_corners=True)
        latents = self.image_encoder(image)
        #latents = self.latents_porj(latents)
        latents = latents.to(dtype=self.dtype)
        res = torch.randn(latents.size(), generator=generator, device=self.device)
        noise = base_noise_add_ratio * latents + (1 - base_noise_add_ratio) * res
        #self.tr_noise_scheduler.add_noise(latents, noise, self.tr_noise_scheduler.num_train_timesteps.long())
        return noise.to(dtype=self.dtype)
    ...
    def forward_train_multicontrol(self, inputs, outputs):
        ...
        latents = self.image_encoder(image)
        latents = latents.to(dtype=self.dtype)
        # TODO: noise from image
        res_noise = torch.randn_like(latents)
        #base_noise = base_noise_add_ratio * latents + (1 - base_noise_add_ratio) * torch.randn_like(latents)
        # ref_noise = self.gen_ref_noise(ref_image, None, image.shape[2], image.shape[3])
        # noise = noise_ratio * ref_noise + (1 - noise_ratio) * res_noise
        noise = res_noise
        ...
        outputs['loss_total'] = loss #+ loss_sim
        return outputs
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        # ref_noise = self.gen_ref_noise(ref_image, generator, h, w)
        ...
        #     latents=ref_noise,
        # )
        # latents = noise_ratio * base_latents + (1 - noise_ratio) * res_latents
        latents = res_latents
