...
from config import *
from config.stage2_controlnet.net import Net
...
from .register import register_time, register_attention_control_efficient, register_conv_control_efficient
import PIL
base_noise_add_ratio = 0.05
noise_ratio = 0.5
...
class TemConvNet(Net):
    def __init__(self, args):
        ...
        if args.ref_null_caption:  # default to False
            ...
            controlnet_prev = ControlNetModel3D.from_unet_2d(
                unet=unet, args=self.args, use_sd_vae=True
            )
        controlnet_list.append(controlnet_background)
        # controlnet_list.append(controlnet_prev)
        ...
        self.pose_proj = torch.nn.Sequential(
            torch.nn.Conv2d(6, 128, 3, padding=1),
        ...
        self.set_processer(self.unet)

        self.noise_scheduler.set_timesteps(self.args.num_inference_steps, device=self.device)
        pnp_f_t = int(self.args.num_inference_steps * 0.8)
        pnp_attn_t = int(self.args.num_inference_steps * 0.8)
        self.qk_injection_timesteps = self.noise_scheduler.timesteps[:pnp_attn_t] if pnp_attn_t >= 0 else []
        self.conv_injection_timesteps = self.noise_scheduler.timesteps[:pnp_f_t] if pnp_f_t >= 0 else []
    ...
    def set_processer(self, unet):
        attn_processor_dict = {}
        for k, v in unet.attn_processors.items():
            # if k == 'XFormersCrossAttnProcessor':
            #     attn_processor_dict[k] = CrossAttnProcessor()
            # else:
            #     attn_processor_dict[k] = v
            attn_processor_dict[k] = CrossAttnProcessor()
    ...
    def freeze_pretrained_part_in_ddpm(self):
        if getattr(self.args, "stage2_only_pose", False):
            ...
            for param_name, param in self.controlnet.named_parameters():
                if (not 'nets.0' in param_name) and (not 'nets.2' in param_name):
    ...
    def clip_encode_image_local(self, image, num_images_per_prompt=1, do_classifier_free_guidance=False): # clip local feature
        ...
        if do_classifier_free_guidance:
            ...
            image_embeddings = torch.cat([image_embeddings, negative_prompt_embeds, image_embeddings])
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        B = b * self.args.num_inf_images_per_prompt
        ref_image = inputs["reference_img"]
        ref_pose_full = torch.cat([inputs["ref_pose"], inputs["ref_densepose"]], dim=1)
        ref_pose_full = self.pose_proj(ref_pose_full)
        ref_pose_seq = ref_pose_full.unsqueeze(2).repeat(1, 1, self.args.nframes, 1, 1)
        
        do_classifier_free_guidance = self.guidance_scale > 1.0

        ref_cond = inputs["full_pose_img_seq"]
        cond_seq = rearrange(ref_cond, "b c t h w -> (b t) c h w") # actually 4C
        cond_seq = self.pose_proj(cond_seq)
        cond_seq = rearrange(cond_seq, "(b t) c h w -> b c t h w", t=self.args.nframes)
        ...
        image_pose = self.prepare_image(
            image=cond_seq.to(dtype=self.dtype),
            width=w,
            height=h,
        ...
        ref_pose = self.prepare_image(
            image=ref_pose_seq.to(dtype=self.dtype),
            width=w,
            height=h,
            batch_size=b * self.args.num_inf_images_per_prompt,
            num_images_per_prompt=self.args.num_inf_images_per_prompt,
        ...
        controlnet_cond_list_ddim_inv.append(ref_pose[2*B:])
        ...
        controlnet_cond_list_ddim_inv.append(reference_latents_controlnet[2*B:])
        ...
        #ddim_inv
        ref_image_resized = inputs["reference_img_raw"]
        if h != 0 and w != 0:
            ref_image_resized = torch.nn.functional.interpolate(ref_image_resized, size=(h, w), mode="bilinear", align_corners=True)
        latent = self.image_encoder(ref_image_resized)
        latent = latent.unsqueeze(2).repeat(1, 1, self.args.nframes, 1, 1)
        latent_list = [latent]
        register_time(self, 0)
        timesteps = reversed(self.noise_scheduler.timesteps)
        for i, t in enumerate(timesteps):
            alpha_prod_t = self.noise_scheduler.alphas_cumprod[t]
            alpha_prod_t_prev = (
                self.noise_scheduler.alphas_cumprod[timesteps[i - 1]]
                if i > 0 else self.noise_scheduler.final_alpha_cumprod
            )
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                latent,
                t,
                encoder_hidden_states=[refer_latents[2*B:]] * len(self.args.conds),
                controlnet_cond=controlnet_cond_list_ddim_inv,
            ...
            eps = self.unet(
                    latent,
                    t,
                    encoder_hidden_states=refer_latents[2*B:],
        ...
        for j in range(1):
            ...
            with self.progress_bar(total=self.args.num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    latent_model_input = (
                        torch.cat([latent_list[-1-i]]+[latents] * 2) if do_classifier_free_guidance else latents
                    )
        ...
        # ref_pose_seq_mask = torch.where(ref_pose_seq > 0, 1.0, 0.0)
        # gen_img = gen_list[0] * ref_pose_seq_mask + gen_list[1] * (1 - ref_pose_seq_mask)
        gen_img = gen_list[0]
