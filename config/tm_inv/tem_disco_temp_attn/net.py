...
class TemConvNet(Net):
    def __init__(self, args):
        ...
        pnp_attn_t = int(self.args.num_inference_steps * 0.2)
    ...
    def clip_encode_image_local(self, image, num_images_per_prompt=1, do_classifier_free_guidance=False): # clip local feature
        ...
        if do_classifier_free_guidance:
            ...
            image_embeddings = torch.cat([negative_prompt_embeds, negative_prompt_embeds, image_embeddings])
    ...
    def forward_sample_multicontrol(self, inputs, outputs):
        ...
        ref_pose_seq = inputs["ref_densepose"].unsqueeze(2).repeat(1, 1, self.args.nframes, 1, 1)
        ...
        # Denoising loop
        res_latents = self.prepare_latents(
        ...
        # ref_noise_single = self.gen_ref_noise(ref_image, generator, h, w)
        # ref_noise = torch.unsqueeze(ref_noise_single, 2).repeat(1, 1, self.args.nframes, 1, 1)
        ...
        #     latents=ref_noise,
        # )
        # res_latents = noise_ratio * base_latents + (1 - noise_ratio) * res_latents

        gen_list = []
        for j in range(1):
            latents = latent_list[-1] if j == 0 else res_latents
